...
'''
Processed data for controls 
'''

#Process inicial dataframe
df_raw_control = pd.read_csv(r'C:\Users\belen\Desktop\Université de Lorraine\703\Aphasia\Data\Cleaned_files\control_data_clean.csv')

processed_df_control = split_into_clauses(df_raw_control, 'text')

#Clause_length metric
processed_df_control['Word_Counts'] = processed_df_control['Split_Clauses'].apply(count_words_in_clause_list_and_return_list)

#Depth_tree metric
processed_df_control['Max_Syntax_Tree_Depth'] = processed_df_control['Split_Clauses'].apply(calculate_max_tree_depth)

#Word_order metric
processed_df_control['Word_Order'] = processed_df_control['Split_Clauses'].apply(get_word_order_for_clauses)

#Verb_phrases metric
processed_df_control['Verb_Phrases'] = processed_df_control['Split_Clauses'].apply(get_verb_phrase_counts_for_clauses)

print(processed_df.head())

#Save data
path= r'C:\Users\belen\Desktop\Université de Lorraine\703\Aphasia\Data\Processed_data\syntax_data_processed_control.csv'

processed_df_control.to_csv(path, index=False)